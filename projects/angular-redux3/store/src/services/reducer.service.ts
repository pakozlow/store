 ...
 *
 * @Action
 * isLogin(state: Auth, action: UnknownAction) {
 ...
 *
 * //old-way
 * export function authReducer(state: Auth, action: UnknownAction): Auth {
...
export class ReducerService {
    ...
    composeReducers(rootReducer: Reducer, middlewares: Middleware[] = []): Reducer {
        ...
        return (state: any, action: UnknownAction): any => {
     ...
     * @param {UnknownAction} action - The current action dispatched to the sub-store.
    ...
    private subStoreRootReducer(state: any, action: UnknownAction, next: NextMiddleware): UnknownAction {
     ...
     * @param {UnknownAction} action - The current action being dispatched.
    ...
    private executeMiddlewareChain(state: any, action: UnknownAction, middlewareList: Middleware[]): any {
        const middlewares = middlewareList.slice();

        function next(newState: any, newAction?: UnknownAction): any {
            if (!newAction) {
                newAction = action;
            }
